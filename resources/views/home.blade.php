...
@php
    ...
    $heroBgImage = $heroSection?->getImageSetting("backgroundImage", $btcUrl . "/wp-content/uploads/2025/12/488478569_2174213226369837_7066166916975308288_n.jpg");
...
@php
    ...
    $awardSideImage = $awardSection?->getImageSetting("sideImage", $btcUrl . "/wp-content/uploads/2025/12/palay-08.jpg");
...
@php
    // Get Seasonal CTA Section Settings
    $seasonalSection = $sections->get("seasonal_cta");
    $seasonalVideoUrl = $seasonalSection?->getSetting("videoUrl", "https://www.youtube.com/embed/qVk6thNT_uM");
    $seasonalSupertext = $seasonalSection?->getSetting("supertext", "Maximum Income and Sustainability");
    $seasonalTitle = $seasonalSection?->getSetting("title", "Reach Your Crop's Maximum Potential for the Season");
    $seasonalDescription = $seasonalSection?->getSetting("description", "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.");
    $seasonalCtaText = $seasonalSection?->getSetting("ctaText", "Join Our Community");
    $seasonalCtaUrl = $seasonalSection?->getSetting("ctaUrl", "/courses");
    // Get carousel items from the section
...
<section class="relative py-16 md:py-24 overflow-hidden" style="background-color: #2d5016;">
    ...
    <div class="absolute inset-0 overflow-hidden">
        @php
            ...
            $videoId = 'qVk6thNT_uM';
            if (preg_match('/embed\/([^?]+)/', $seasonalVideoUrl, $matches)) {
                $videoId = $matches[1];
            }
        @endphp
        <iframe
            ...
            images: [
                @if($carouselItems->isNotEmpty())
                    @foreach($carouselItems as $item)
                        { src: '{{ $item->imageUrl }}', alt: '{{ $item->title ?? "Crop" }}' }{{ !$loop->last ? ',' : '' }}
                    @endforeach
                @else
                    {{-- Fallback images if no carousel items configured --}}
